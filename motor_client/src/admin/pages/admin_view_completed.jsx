...
function ViewCompleted() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const response = await axios.get('http://localhost:5000/api/ViewCompleted'); // Assuming your backend endpoint is '/api/assistance-roads'
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', border: '1px solid #f7d560' }} id="table-container">
        <thead>
          ...
          <th style={{ backgroundColor: '#36454f', border: '1px solid #f7d560', padding: '8px', textAlign: 'left' }}>ID</th>
            ...
            <th style={{ backgroundColor: '#36454f', border: '1px solid #f7d560', padding: '8px', textAlign: 'left' }}>Longitude</th>

            <th style={{ backgroundColor: '#36454f', border: '1px solid #f7d560', padding: '8px', textAlign: 'left' }}>Latitude</th>
            <th style={{ backgroundColor: '#36454f', border: '1px solid #f7d560', padding: '8px', textAlign: 'left' }}>Status</th>
            ...
            <th style={{ backgroundColor: '#36454f', border: '1px solid #f7d560', padding: '8px', textAlign: 'left' }}>users contact</th>
            <th style={{ backgroundColor: '#36454f', border: '1px solid #f7d560', padding: '8px', textAlign: 'left' }}>users VIN</th>
        ...
        <tbody>
          {filteredRoads.map((road) => (
            <tr key={road._id}>
              ...
              <td style={{ border: '1px solid #f7d560', padding: '8px', textAlign: 'left' }}>{road.longitude}</td>
              <td style={{ border: '1px solid #f7d560', padding: '8px', textAlign: 'left' }}>{road.latitude}</td>
              <td style={{ border: '1px solid #f7d560', padding: '8px', textAlign: 'left' }}>{road.status}</td>
              ...
              <td style={{ border: '1px solid #f7d560', padding: '8px', textAlign: 'left' }}>{road.phoneNumber}</td>
              <td style={{ border: '1px solid #f7d560', padding: '8px', textAlign: 'left' }}>{road.vehicleRegNumber}</td>

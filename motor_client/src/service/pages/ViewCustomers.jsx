  ...
  useEffect(() => {
    ...
    axios.get(`http://localhost:5000/api/emp/${mail}`)
      .then((response) => {
        ...
        axios.get(`http://localhost:5000/api/bookedservice/${response.data._id}`)
    ...
    <Card sx={{ maxWidth: 400, margin: 'auto', marginTop: 10, backgroundColor: '#D4AF37' }}>
      <CardContent>
        ...
        <div key={index}>
            ...
            <Typography>Model: {bookings.Models}</Typography>
            <Typography>pickupStatus: {bookings.pickupstatus}</Typography>
            <Typography>Booking Date: {bookings.bookingDate}</Typography>
            <Typography> pickupAddress: {bookings.pickupAddress}</Typography>
            <Typography> selectedOptions: {bookings.selectedOptions}</Typography>
